...
``salted_prediction`` parallelises over the atoms of a single structure.
Run it inside a trained SALTED workspace (the cwd must contain inp.yaml):
...
def main():
    ...
    if size > 1:
        comm.Barrier()
        coefs = comm.allreduce(coefs)  # sum the per-rank atomic blocks
    if rank == 0:
        np.save(out_fpath, coefs)
