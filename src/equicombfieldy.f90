SUBROUTINE equicombfieldy(natoms,nang1,nrad1,nrad2,v1,v2,wigdim,w3j,llmax,llvec,lam,c2r,p)
...
COMPLEX*16, DIMENSION(nrad2,natoms):: v2 
REAL*8, DIMENSION(2*lam+1,llmax,nrad2,nrad1,natoms):: p 

!f2py intent(in) natoms,nang1,nrad1,nrad2,v1,v2,wigdim,w3j,llmax,llvec,lam,c2r
!f2py intent(out) p 
...
!$OMP FIRSTPRIVATE(natoms,nang1,nrad1,nrad2,w3j,llmax,llvec,lam,c2r) &
!$OMP SHARED(p,v1,v2)
         ...
         do il=1,llmax
            ...
            do imu=1,2*lam+1 
               ...
               do im1=1,2*l1+1
                  m1 = im1-1-l1
                  m2 = m1-mu
                  if (abs(m2)<=l2 .and. m2==1) then 
                     im2 = m2+l2+1
                     pcmplx(imu) = pcmplx(imu) &
                                 + w3j(iwig) * v1(im1,l1+1,n1,iat) * dconjg(dcmplx(0.d0,-1.d0)*v2(n2,iat))
                     iwig = iwig + 1
                  endif
               enddo
            enddo
