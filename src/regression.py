...
#            regs = np.array([1e-04,1e-05,1e-06,1e-07,1e-08,1e-09,1e-10,1e-11,1e-12,1e-13,1e-14])
#            jits = np.array([1e-04,1e-05,1e-06,1e-07,1e-08,1e-09,1e-10,1e-11,1e-12,1e-13,1e-14])
#            error_opt = 1000.0
#              
#            # do regression
#            for reg in regs:
#                for jit in jits:
            ...
            #    print >> f, ortho_projs, ortho_projs_ref
                # compute error
            ...
            #sys.exit(0)
            #print "-------------------------"
            #print "Error =", np.sqrt(error_total/ntest)
            #print "STD =", np.sqrt(variance/ntest)
            print "% RMSE =", 100*np.sqrt(error_total/variance)
            #        rmse = 100*np.sqrt(error_total/variance)
            #        if rmse<error_opt:
            #            regg = reg.copy()
            #            jitt = jit.copy()
            #            error_opt = rmse.copy()
            #            preds_opt = preds.copy()
            #print regg, jitt, error_opt 

np.save("predictions.npy",preds)
#np.save("predictions.npy",preds_opt)
