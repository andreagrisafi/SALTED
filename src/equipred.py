...
for lam in range(lmax_max+1):
    pvec_train[lam] = np.load(inp.saltedpath+"equirepr_"+inp.saltedname+"/FEAT-"+str(lam)+".npy")
    for spe in species:
    ...
    if lam==0:
        ...
        for iconf in range(ndata):
            for spe in species:
                ...
                # Project on RKHS
                psi_nm[(iconf,spe,lam)] = np.real(np.dot(kernel_nm,Vmat[(lam,spe)]))    
    else:

        # Compute covariant kernels
        for iconf in range(ndata):
            for spe in species:
                kernel_nm = np.dot(pvec[iconf,atom_idx[(iconf,spe)]].reshape(natom_dict[(iconf,spe)]*(2*lam+1),featspace),power_env_sparse[(lam,spe)].T)
