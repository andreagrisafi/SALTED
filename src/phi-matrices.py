        ...
        for n in xrange(nmax[(spe,l)]):
            ...
            for spe2 in spelist:
                for l2 in xrange(lmax[spe2]+1):
                    Mcut[(spe2,l2)] = np.load(inp.path2ml+kdir+"spe"+str(spe2)+"_l"+str(l2)+"/M"+str(M)+"_eigcut"+str(int(np.log10(eigcut)))+"/psi-nm_conf"+str(0)+".npy").shape[1]
...
for iconf in trainrange:
    ...
    for spe in spelist:
        ispe[spe] = 0
        for l in xrange(lmax[spe]+1):
            psi_nm[(spe,l)] = np.load(inp.path2ml+kdir+"spe"+str(spe)+"_l"+str(l)+"/M"+str(M)+"_eigcut"+str(int(np.log10(eigcut)))+"/psi-nm_conf"+str(iconf)+".npy")
