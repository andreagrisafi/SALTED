...
COMPLEX*16, DIMENSION(3,natoms,2*nang1+1,nang1+1,nrad1,natoms):: dv1
COMPLEX*16, DIMENSION(3,natoms,2*nang2+1,nang2+1,nrad2,natoms):: dv2
         ...
         do il=1,llmax
               ...
               do im1=1,2*l1+1
                  ...
                  if (abs(m2)<=l2) then
                     ...
                     pcmplx(imu) = pcmplx(imu) &
                                 + w3j(iwig) * v1v * v2c
                     do i_grad=1,natoms
                        dv2c1 = dconjg(dv2(1,i_grad,im2,l2+1,n2,iat))
                        dv2c2 = dconjg(dv2(2,i_grad,im2,l2+1,n2,iat))
                        dv2c3 = dconjg(dv2(3,i_grad,im2,l2+1,n2,iat))
                        grad_pcmplx(1,i_grad,imu) = grad_pcmplx(1,i_grad,imu) &
                                 + w3j(iwig) * (dv1(1,i_grad,im1,l1+1,n1,iat) * v2c&
                                 + v1v * dv2c1)
                        grad_pcmplx(2,i_grad,imu) = grad_pcmplx(2,i_grad,imu) &
                                 + w3j(iwig) * (dv1(2,i_grad,im1,l1+1,n1,iat) * v2c&
                                 + v1v * dv2c2)
                        grad_pcmplx(3,i_grad,imu) = grad_pcmplx(3,i_grad,imu) &
                                 + w3j(iwig) * (dv1(3,i_grad,im1,l1+1,n1,iat) * v2c&
                                 + v1v * dv2c3)
                     enddo
                     iwig = iwig + 1
                  endif
            ...
            do imu = 1, 2*lam+1
               do im1 = 1, 2*lam+1
                   ...
                   do i_grad = 1, natoms
                       grad_preal(1,i_grad,imu) = grad_preal(1,i_grad,imu) + &
                       dreal(c2r(imu,im1) * grad_pcmplx(1,i_grad,im1))
                       grad_preal(2,i_grad,imu) = grad_preal(2,i_grad,imu) + &
                       dreal(c2r(imu,im1) * grad_pcmplx(2,i_grad,im1))
                       grad_preal(3,i_grad,imu) = grad_preal(3,i_grad,imu) + &
                       dreal(c2r(imu,im1) * grad_pcmplx(3,i_grad,im1))
                   enddo
               enddo
               inner = inner + preal(imu)**2
               ptemp(imu,ifeat) = preal(imu)
               do i_grad=1,natoms
                 dot1(i_grad) = dot1(i_grad) + preal(imu)*grad_preal(1,i_grad,imu)
                 dot2(i_grad) = dot2(i_grad) + preal(imu)*grad_preal(2,i_grad,imu)
                 dot3(i_grad) = dot3(i_grad) + preal(imu)*grad_preal(3,i_grad,imu)
                 grad_ptemp(1,i_grad,imu,ifeat) = grad_preal(1,i_grad,imu)
                 grad_ptemp(2,i_grad,imu,ifeat) = grad_preal(2,i_grad,imu)
                 grad_ptemp(3,i_grad,imu,ifeat) = grad_preal(3,i_grad,imu)
               enddo
            enddo
            ifeat = ifeat + 1
         enddo
