...
print("Reconstructing the electron density in real space...")
...
print("number of auxiliary functions =", naux)

# load RI-coefficients
coefs = np.load(inp.path2ml+pdir+"M"+str(inp.Menv)+"_eigcut"+str(int(np.log10(inp.eigcut)))+"/N"+str(inp.Ntrain)+"_reg"+str(int(np.log10(reg)))+"/prediction_conf"+str(iconf)+".npy")
...
dirpath = os.path.join(inp.path2ml, cdir)
if not os.path.exists(dirpath):
    os.mkdir(dirpath)

dirpath = os.path.join(inp.path2ml+cdir, "M"+str(M)+"_eigcut"+str(int(np.log10(eigcut))))
if not os.path.exists(dirpath):
    os.mkdir(dirpath)

dirpath = os.path.join(inp.path2ml+cdir+"M"+str(M)+"_eigcut"+str(int(np.log10(eigcut)))+"/","N"+str(ntrain))+"_reg"+str(int(np.log10(reg)))
if not os.path.exists(dirpath):
    os.mkdir(dirpath)


# pring density on a cube file
filename = "rhor_conf-"+str(iconf+1)+".cube"
cubef = open(inp.path2ml+cdir+"M"+str(M)+"_eigcut"+str(int(np.log10(eigcut)))+"/N"+str(ntrain)+"_reg"+str(int(np.log10(reg)))+"/"+filename,"w")
...
if totcharge:
    ...
    filename = "rhoQ_conf-"+str(iconf+1)+".cube"
    cubef = open(inp.path2ml+cdir+"M"+str(M)+"_eigcut"+str(int(np.log10(eigcut)))+"/N"+str(ntrain)+"_reg"+str(int(np.log10(reg)))+"/"+filename,"w")
    print("Reconstructed electron density",file=cubef)
    print("CUBE FORMAT",file=cubef)
    print(natoms, origin[0], origin[1], origin[2],file=cubef)
