...
# setuptools: availability and version
...
ifeq ($(SETUPTOOLS_CHECK),fail)
	$(error setuptools module is not installed. Please install it.)
endif
...
USE_MESON := $(shell python -c "from packaging import version; import numpy; print('yes' if version.parse(numpy.__version__) >= version.parse('1.23.0') else 'no')" 2>/dev/null || echo "no")

# Check module version, set backend flag and compiler configuration
ifeq ($(USE_MESON),yes)
    ifeq ($(shell which meson >/dev/null 2>&1 && echo "yes" || echo "no"),no)
        $(error NumPy >= 1.23 requires meson backend, but meson is not installed. Please install meson and ninja.)
    endif
    MESON_VERSION := $(shell meson --version 2>/dev/null || echo "Cannot determine meson version...")
    # Meson backend: use FC environment variable, not --fcompiler
    F2PY_COMPILER_VARS := FC='gfortran' FFLAGS='-fopenmp'
    BACKEND_FLAG := --backend meson
    F2PY_COMPILER_FLAGS := --f90flags='-fopenmp -O2'
    F2PY_LIBS := -lgomp
    ...
    F2PY_COMPILER_VARS :=
    BACKEND_FLAG := --backend distutils
    F2PY_COMPILER_FLAGS := --fcompiler='gfortran' --f90flags='-fopenmp -O2'
    F2PY_LIBS := -lgomp
    ...
    ifeq ($(SETUPTOOLS_OK),no)
        $(warning setuptools >= 60.0 detected ($(SETUPTOOLS_VERSION)). Consider downgrading to setuptools<60.0 for distutils backend compatibility.)
    endif
endif
...
salted/lib/%.so: src/%.f90
	cd salted/lib; $(F2PY_COMPILER_VARS) python -m numpy.f2py $(F2PYOPT) -c ../../src/$*.f90 -m $* $(BACKEND_FLAG) $(F2PY_COMPILER_FLAGS) $(F2PY_LIBS) --quiet; mv $*.*.so $*.so
