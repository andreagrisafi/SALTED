...
| `species` | `List[str]` | **Required** | List of element species. |
| `average` | `bool` | `False` | Whether we use averaged coefficients to set an offset. Normally this should be true, unless a delta-density is learned. |
...
| `sig` | `float` | **Required** | Gaussian function width (Angstrom) for atomic density. |
...
| `ncut` | `int` | `0` | Sets maximum number of SOAP features kept. `0` for no sparsification. |
...
| var name | type | default | usage |
| -:| :-: | :-: | :- |
| `z` | `float` | `2.0` | Kernel exponent $\chi$ for GPR. |
...
| `trainsel` | `Union[Literal["sequential"], Literal["random"]]` | `"random"` | Train at random or sequentially for MPI matrix inversion. |
